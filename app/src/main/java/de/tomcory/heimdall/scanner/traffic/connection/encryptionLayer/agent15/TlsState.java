 ...
 */
package de.tomcory.heimdall.scanner.traffic.connection.encryptionLayer.agent15;
...
import de.tomcory.heimdall.scanner.traffic.connection.encryptionLayer.agent15.TranscriptHash;
...
public class TlsState {
    ...
    public TlsState(TranscriptHash transcriptHash, byte[] psk, int keyLength, int hashLength) {
        ...
        de.tomcory.heimdall.scanner.traffic.connection.encryptionLayer.agent15.Logger.debug("Empty hash: " + bytesToHex(emptyHash));
    ...
    private byte[] computeEarlySecret(byte[] ikm) {
        ...
        de.tomcory.heimdall.scanner.traffic.connection.encryptionLayer.agent15.Logger.debug("Early secret: " + bytesToHex(earlySecret));

        binderKey = hkdfExpandLabel(earlySecret, "res binder", emptyHash, hashLength);
        de.tomcory.heimdall.scanner.traffic.connection.encryptionLayer.agent15.Logger.debug("Binder key: " + bytesToHex(binderKey));
...
//                com.android.org.conscrypt.OpenSSLX25519PublicKey;
            ...
            de.tomcory.heimdall.scanner.traffic.connection.encryptionLayer.agent15.Logger.debug("Shared key: " + bytesToHex(sharedSecret));
        }
    ...
    public void computeHandshakeSecrets() {
        ...
        de.tomcory.heimdall.scanner.traffic.connection.encryptionLayer.agent15.Logger.debug("Derived secret: " + bytesToHex(derivedSecret));

        handshakeSecret = hkdf.extract(derivedSecret, sharedSecret);
        de.tomcory.heimdall.scanner.traffic.connection.encryptionLayer.agent15.Logger.debug("Handshake secret: " + bytesToHex(handshakeSecret));
        ...
        de.tomcory.heimdall.scanner.traffic.connection.encryptionLayer.agent15.Logger.debug("Client handshake traffic secret: " + bytesToHex(clientHandshakeTrafficSecret));

        serverHandshakeTrafficSecret = hkdfExpandLabel(handshakeSecret, "s hs traffic", handshakeHash, hashLength);
        de.tomcory.heimdall.scanner.traffic.connection.encryptionLayer.agent15.Logger.debug("Server handshake traffic secret: " + bytesToHex(serverHandshakeTrafficSecret));

        byte[] clientHandshakeKey = hkdfExpandLabel(clientHandshakeTrafficSecret, "key", "", keyLength);
        de.tomcory.heimdall.scanner.traffic.connection.encryptionLayer.agent15.Logger.debug("Client handshake key: " + bytesToHex(clientHandshakeKey));

        byte[] serverHandshakeKey = hkdfExpandLabel(serverHandshakeTrafficSecret, "key", "", keyLength);
        de.tomcory.heimdall.scanner.traffic.connection.encryptionLayer.agent15.Logger.debug("Server handshake key: " + bytesToHex(serverHandshakeKey));

        byte[] clientHandshakeIV = hkdfExpandLabel(clientHandshakeTrafficSecret, "iv", "", iv_length);
        de.tomcory.heimdall.scanner.traffic.connection.encryptionLayer.agent15.Logger.debug("Client handshake iv: " + bytesToHex(clientHandshakeIV));

        byte[] serverHandshakeIV = hkdfExpandLabel(serverHandshakeTrafficSecret, "iv", "", iv_length);
        de.tomcory.heimdall.scanner.traffic.connection.encryptionLayer.agent15.Logger.debug("Server handshake iv: " + bytesToHex(serverHandshakeIV));
    }
    ...
    void computeApplicationSecrets(byte[] handshakeSecret) {
        ...
        de.tomcory.heimdall.scanner.traffic.connection.encryptionLayer.agent15.Logger.debug("Master secret: "+ bytesToHex(masterSecret));

        clientApplicationTrafficSecret = hkdfExpandLabel(masterSecret, "c ap traffic", serverFinishedHash, hashLength);
        de.tomcory.heimdall.scanner.traffic.connection.encryptionLayer.agent15.Logger.debug("Client application traffic secret: " + bytesToHex(clientApplicationTrafficSecret));

        serverApplicationTrafficSecret = hkdfExpandLabel(masterSecret, "s ap traffic", serverFinishedHash, hashLength);
        de.tomcory.heimdall.scanner.traffic.connection.encryptionLayer.agent15.Logger.debug("Server application traffic secret: " + bytesToHex(serverApplicationTrafficSecret));

        byte[] clientApplicationKey = hkdfExpandLabel(clientApplicationTrafficSecret, "key", "", keyLength);
        de.tomcory.heimdall.scanner.traffic.connection.encryptionLayer.agent15.Logger.debug("Client application key: " + bytesToHex(clientApplicationKey));

        byte[] serverApplicationKey = hkdfExpandLabel(serverApplicationTrafficSecret, "key", "", keyLength);
        de.tomcory.heimdall.scanner.traffic.connection.encryptionLayer.agent15.Logger.debug("Server application key: " + bytesToHex(serverApplicationKey));

        byte[] clientApplicationIv = hkdfExpandLabel(clientApplicationTrafficSecret, "iv", "", iv_length);
        de.tomcory.heimdall.scanner.traffic.connection.encryptionLayer.agent15.Logger.debug("Client application iv: " + bytesToHex(clientApplicationIv));

        byte[] serverApplicationIv = hkdfExpandLabel(serverApplicationTrafficSecret, "iv", "", iv_length);
        de.tomcory.heimdall.scanner.traffic.connection.encryptionLayer.agent15.Logger.debug("Server application iv: " + bytesToHex(serverApplicationIv));
    }
