...
import de.tomcory.heimdall.scanner.traffic.connection.encryptionLayer.agent15.extension.ExtensionParser;
...
import static de.tomcory.heimdall.scanner.traffic.connection.encryptionLayer.agent15.TlsConstants.HandshakeType.*;
...
public class TlsMessageParser {
    ...
    public de.tomcory.heimdall.scanner.traffic.connection.encryptionLayer.agent15.handshake.HandshakeMessage parseAndProcessHandshakeMessage(ByteBuffer buffer, MessageProcessor messageProcessor, ProtectionKeysType protectedBy) throws TlsProtocolException, IOException {
        ...
        if (messageType == client_hello.value) {
            de.tomcory.heimdall.scanner.traffic.connection.encryptionLayer.agent15.handshake.ClientHello ch = new ClientHello(buffer, customExtensionParser);
            parsedMessage = ch;
            messageProcessor.received(ch, protectedBy);
        }
        else if (messageType == server_hello.value) {
            de.tomcory.heimdall.scanner.traffic.connection.encryptionLayer.agent15.handshake.ServerHello sh = new ServerHello().parse(buffer, length + 4);
            parsedMessage = sh;
            ...
            de.tomcory.heimdall.scanner.traffic.connection.encryptionLayer.agent15.handshake.FinishedMessage fm = new FinishedMessage().parse(buffer, length + 4);
            parsedMessage = fm;
