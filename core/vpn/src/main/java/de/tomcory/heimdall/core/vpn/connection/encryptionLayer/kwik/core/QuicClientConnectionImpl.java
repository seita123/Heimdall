...
import static de.tomcory.heimdall.core.vpn.connection.encryptionLayer.kwik.QuicConstants.TransportErrorCode.PROTOCOL_VIOLATION;
import static de.tomcory.heimdall.core.vpn.connection.encryptionLayer.kwik.QuicConstants.TransportErrorCode.TRANSPORT_PARAMETER_ERROR;
import static de.tomcory.heimdall.core.vpn.connection.encryptionLayer.kwik.QuicConstants.TransportErrorCode.VERSION_NEGOTIATION_ERROR;
import static de.tomcory.heimdall.core.vpn.connection.encryptionLayer.kwik.core.EncryptionLevel.App;
import static de.tomcory.heimdall.core.vpn.connection.encryptionLayer.kwik.core.EncryptionLevel.Handshake;
import static de.tomcory.heimdall.core.vpn.connection.encryptionLayer.kwik.core.EncryptionLevel.Initial;
...
import de.tomcory.heimdall.core.vpn.connection.encryptionLayer.agent15.extension.ApplicationLayerProtocolNegotiationExtension;
import de.tomcory.heimdall.core.vpn.connection.encryptionLayer.agent15.extension.EarlyDataExtension;
import de.tomcory.heimdall.core.vpn.connection.encryptionLayer.agent15.extension.Extension;
import de.tomcory.heimdall.core.vpn.connection.encryptionLayer.agent15.handshake.*;
import de.tomcory.heimdall.core.vpn.connection.encryptionLayer.kwik.DatagramSocketFactory;
import de.tomcory.heimdall.core.vpn.connection.encryptionLayer.kwik.QuicClientConnection;
import de.tomcory.heimdall.core.vpn.connection.encryptionLayer.kwik.QuicSessionTicket;
...
import de.tomcory.heimdall.core.vpn.connection.encryptionLayer.kwik.cid.ConnectionIdInfo;
...
import de.tomcory.heimdall.core.vpn.connection.encryptionLayer.kwik.stream.EarlyDataStream;
...
import de.tomcory.heimdall.core.vpn.connection.encryptionLayer.kwik.tls.QuicTransportParametersExtension;
...
//        receiver.start();
        ...
        if (!earlyData.isEmpty()) {
            if (earlyDataStatus != EarlyDataStatus.Accepted) {
                log.info("Server did not accept early data; retransmitting all data.");
            }
            for (QuicStream stream: earlyDataStreams) {
                if (stream != null) {
                    ((EarlyDataStream) stream).writeRemaining(earlyDataStatus == EarlyDataStatus.Accepted);
                }
            }
        }
    ...
    private List<QuicStream> sendEarlyData(List<StreamEarlyData> streamEarlyDataList) throws IOException {
        if (!streamEarlyDataList.isEmpty()) {
            ...
            earlyDataStatus = EarlyDataStatus.Requested;
            return earlyDataStreams;
        }
        else {
